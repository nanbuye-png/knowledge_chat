...
export default function SystemConfig() {
  ...
  const updateField = (sectionIdx: number, fieldIdx: number, value: string) => {
    setSections(prev => {
      const updated = [...prev]
      updated[sectionIdx] = { ...updated[sectionIdx], fields: [...updated[sectionIdx].fields] }
      updated[sectionIdx].fields[fieldIdx] = { ...updated[sectionIdx].fields[fieldIdx], value }
      return updated
    })
  }
    ...
    <div className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          <button onClick={fetchConfig} className="p-2 rounded-lg text-slate-400 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-700">
      ...
      {msg && (
        <div className={`mb-4 p-3 rounded-xl text-sm ${msg.includes('失败') ? 'bg-red-50 text-red-700 dark:bg-red-900/20 dark:text-red-300' : 'bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-300'}`}>
          {msg}
        </div>
      )}
